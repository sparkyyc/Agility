...
import { Form, Button, Icon, Label, Search, Dimmer, Loader, Rating, Header, Segment } from 'semantic-ui-react'
...
class PersonAddSkills extends React.Component{
    ...
    handleSkillAdd = (event) => {
        ...
        const skillToRate = this.props.data.allSkills.nodes.find(skill => skill.title === this.state.skill)
        if(!skillToRate){
            ...
            this.props.mutate({
                ...
                this.setState({ addedSkills: [...this.state.addedSkills, { id, skillName }]})
            })
        } else {
            this.setState({ addedSkills: [...this.state.addedSkills, { id: skillToRate.id, skillName: skillToRate.title }] })
        }
    ...
    createRatingEl = () => {
        ...
        if(addedSkills.length > 0){
            return addedSkills.map(skill => {
                ...
                const currUserRating = selfRatings[skillName] ? selfRatings[skillName].rating : 0
                return(
                    <Segment key={id} >
                        <Header 
                        dividing
                        content={skillName}
                        subheader='Rate Your Skill'
                        />
                        <Rating
                            maxRating={5}
    ...
    resetComponent = () => this.setState({ isLoading: false, results: [], skill: '' })
...
graphql(fetchSkills)(PersonAddSkills)
)